...
export default {
  ...
  methods: {
    fetchVolunteers() {
      axios.get('http://127.0.0.1:4523/m2/3206870-0-default/118198285') // 请替换为实际的 API 接口地址
        .then(response => {
    ...
    editVolunteer(volunteer) {
      ...
      // 将志愿活动信息传递给编辑页面或弹窗进行修改
      // 可以使用路由跳转或打开一个弹窗组件来进行编辑
      // 例如：
      // this.$router.push({ path: '/edit-volunteer', query: { vol_id: volunteer.vol_id } });
      // 或
      // this.$emit('open-edit-dialog', volunteer.vol_id);
    },
