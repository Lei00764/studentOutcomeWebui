...
import api from '@/api/paper'  // 修改API路径
...
const searchableFields = [
    ...
    { field: "paper_title", name: "论文标题", canApproximate: true },
    {
...
const getPaper = () => {
    if (studentQuery.value) {
        ...
        api.checkApi.getPaperWithStudent(
            userId,
            currentPage.value
        ).then(res => {
        ...
        api.checkApi.getPaperWithKeyword(
            fields,
            currentPage.value
        ).then(res => {
            content.v = res.json.content
...
<template>
    <div class="viewWrapper">
        <h1 class="pageTitle">审核论文记录</h1>
        <div class="helpText">
            帮助：在本页面中，您可以查询并审核学生提交的论文记录。查询条件间的关系为“与”
        </div>
        <div>
            <el-form :inline="true" class="queryForm">
                <el-form-item label="查询模式">
                    <el-switch v-model="studentQuery" class="mb-2"
                        style="--el-switch-on-color: #13ce66; --el-switch-off-color: #13ce66" active-text="按学生查询"
                        inactive-text="全局查询" />
                </el-form-item>
            </el-form>
        ...
        <div>
            <div v-if="!studentQuery">
                <el-form v-for="sf in selectedFields" :inline="true" class="queryForm">
                    <el-form-item label="查询字段">
                        <el-select v-model="sf.sf.field" clearable @change="onFieldSelected(sf)">
                    ...
                    <el-form-item label="选择" v-else :label-width="54">
                        <el-select v-model="sf.sf.keyword" placeholder="无" clearable>
                            <el-option v-for="op in sf.f.selections" :label="op.name" :value="op.value"
                                :key="op.value" />
                        </el-select>
                    </el-form-item>
            ...
            <el-form>
                <el-form-item>
                    ...
                    <el-button v-else @click="addField"
                        :disabled="selectedFields.length >= searchableFields.length">添加条件</el-button>
...
:deep(.queryForm) .el-input,
:deep(.queryForm) .el-select {
