...
const searchableFields = [
    ...
    { field: "patent_title", name: "专利名称", canApproximate: true },
...
const getPatent = () => {
    if (studentQuery.value) {
        ...
        api.checkApi.getPatentWithStudent(
            userId,
            currentPage.value
        ).then(res => {
        ...
        api.checkApi.getPatentWithKeyword(
            fields,
            currentPage.value
        ).then(res => {
            console.log("看这里",res.json.content)
...
const goStartCheck = (row) => {
    ...
    router.push("/patentCheck/" + row.id)
}
