...
const fields = [
    { field: "volunteer_title", name: "志愿者名称" },
]
...
const getVolunteer = () => {
    if (studentQuery.value) {
        ...
        api.checkApi.getVolunteerWithStudent(
            userId,
            currentPage.value
        ).then(res => {
        ...
        api.checkApi.getVolunteerWithKeyword(
            fields,
            currentPage.value
        ).then(res => {
            content.v = res.json.content
...
<template>
    <div class="viewWrapper">
        <h1 class="pageTitle">审核志愿者记录</h1>
        <div class="helpText">
            帮助：在本页面中，您可以查询并审核学生提交的志愿者记录。查询条件间的关系为“与”
        </div>
        <div>
